...
const Editor = ({onClose,previewUrl,settings,setSettings}:{
      onClose:()=>void,
      previewUrl:string;
      settings:{
      ...
      setSettings: Dispatch<SetStateAction<{ type: "original" | "wide" | "square";sensitive: boolean }>>;
}) => {

      const handleChangeSensitive =(sensitive:boolean)=>{
            setSettings((prv)=>({...prv,sensitive}))
      }
      const handleChangeType = (type:"original"|"wide"|"square")=>{
            setSettings((prv)=>({...prv,type}))

      }
      return (
          <div className='fixed w-screen h-screen left-0 top-0 bg-black bg-opacity-75 z-10 items-center justify-center flex'>
                <div className='bg-black rounded-xl p-12 flex flex-col gap-4'>

                      <div className='flex items-center justify-between'>
                            <div className='flex items-center gap-8'>
                                  <svg onClick={onClose} width={32} className='cursor-pointer'
                                       viewBox="0 0 24 24">
                                        <path
                                            fill="#e7e9ea"
